...
import { editBooking, getClericBookings } from '../services'
...
const ClericBookings = () => {

    const { id } = useParams()
    useEffect(() => {
        getClericBookings(id).then(data => setBookings(data.bookings))
    }, [])
    ...
    const sendMessage = (payload) => {
        if (message) {
            editBooking({ id: payload?.id, body: { message: { role: 'cleric', message } } }).then(data => {
